# Agent Evidence v0.1.5

Agent Evidence v0.1.5 is a patch release that refreshes the Agent standards ecosystem after Agent Artifact became a current published standard.

## Highlights

- Adds Agent Artifact to the current standards table and boundary map.
- Removes Agent Artifact from the future-candidate list.
- Adds Agent Artifact to README related-standard links.
- Updates `llms.txt`, `llms-full.txt`, and public LLM copies so AI clients can discover Agent Artifact.
- Adds version snapshot pages for v0.1.5.

## Links

- Documentation: https://limecloud.github.io/agentevidence/
- Agent Artifact: https://limecloud.github.io/agentartifact/
- LLM context: https://limecloud.github.io/agentevidence/llms-full.txt
